...
use reth_provider::{BlockNumReader, ChainSpecProvider, HeaderProvider, StateProviderFactory};
use reth_transaction_pool::test_utils::TransactionBuilder;

fn envelope_from_signed(tx: &OpTransactionSigned) -> Result<OpTxEnvelope> {
    Ok(tx.clone().into())
}

fn create_parsed_bundle(envelopes: Vec<OpTxEnvelope>) -> Result<ParsedBundle> {
...
#[tokio::test]
async fn meter_bundle_empty_transactions() -> Result<()> {
    let harness = TestHarness::new().await?;

    let provider = harness.blockchain_provider();
    let best_num = provider.best_block_number()?;
    let header = provider.sealed_header(best_num)?.expect("header exists");
...
#[tokio::test]
async fn meter_bundle_single_transaction() -> Result<()> {
    let harness = TestHarness::new().await?;
...
#[tokio::test]
async fn meter_bundle_multiple_transactions() -> Result<()> {
    let harness = TestHarness::new().await?;
