...
use base_reth_test_utils::{TestHarness, User};
use eyre::Result;
...
fn chain_spec(harness: &TestHarness) -> Arc<OpChainSpec> {
    harness.blockchain_provider().chain_spec()
}

#[tokio::test]
async fn meter_block_empty_transactions() -> Result<()> {
    let harness = TestHarness::new().await?;

    let block = create_block_with_transactions(&harness, vec![]);

    let response = meter_block(harness.blockchain_provider(), chain_spec(&harness), &block)?;
...
#[tokio::test]
async fn meter_block_single_transaction() -> Result<()> {
    let harness = TestHarness::new().await?;

    let to = Address::random();
    let signed_tx = TransactionBuilder::default()
        .signer(harness.account(User::Alice).private_key_bytes())
...
#[tokio::test]
async fn meter_block_multiple_transactions() -> Result<()> {
    let harness = TestHarness::new().await?;
    ...
    let signed_tx_2 = TransactionBuilder::default()
        .signer(harness.account(User::Bob).private_key_bytes())
        .chain_id(harness.chain_id())
        .nonce(0)
        .to(to_2)
        .value(2_000)
...
#[tokio::test]
async fn meter_block_timing_consistency() -> Result<()> {
    let harness = TestHarness::new().await?;
...
#[tokio::test]
async fn meter_block_parent_header_not_found() -> Result<()> {
    let harness = TestHarness::new().await?;
    let latest = harness.latest_block();
    ...
    let result = meter_block(harness.blockchain_provider(), chain_spec(&harness), &block);
...
#[tokio::test]
async fn meter_block_invalid_transaction_signature() -> Result<()> {
